...
//#endregion
//#region src/node/ruleBlockTab.ts
const tabMarkerCode = "=".charCodeAt(0);
const minTabMarkerLen = 2;
const ruleBlockTab = (state, startLine, endLine, silent) => {
	...
	for (;;) {
		...
		if (endStart < max && state.sCount[nextLine] < state.blkIndent) break;
...
//#endregion
//#region src/node/markdownPlugin.ts
...
const tabsPlugin = (md) => {
	md.use(container_plugin, "tabs", { render(tokens, index) {
		...
		if (token.nesting === 1) {
			...
			return `<PluginTabs ${[params.shareStateKey ? `sharedStateKey="${md.utils.escapeHtml(params.shareStateKey)}"` : "", params.variant ? `variant="${md.utils.escapeHtml(params.variant)}"` : ""].filter(Boolean).join(" ")}>\n`;
		} else return `</PluginTabs>\n`;
	} });
