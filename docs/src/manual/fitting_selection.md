...
`pair_method=:itau` inverts the sample Kendall tau instead of maximizing the
...
`:mle`. The inversion stays inside the same finite selection domains as the
`:mle` candidates. The BB families have two parameters, which tau alone does
not identify, so they are fitted by `:mle` under `:itau`, as in vinecopulib;
`trace=true` prints the method used by each candidate.
