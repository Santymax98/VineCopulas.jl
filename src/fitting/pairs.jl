...
`threaded=true` fits the family × rotation candidates on `Threads.@spawn`
tasks. The selected copula is the one the sequential search returns: the
candidate list is fixed before any fit runs and the winner is read off it in
list order, so a tie resolves to the same candidate on any thread count.
`strict=true` raises the error of the first failing candidate in that order,
and `trace=true` prints the same lines in the same order.
"""
function select_paircopula(U::AbstractMatrix{<:Real}; family_set=:default, pair_method::Symbol=:default, selection_criterion::Symbol=:bic,
                           ...
                           return _select_pair(U; family_set, pair_method, selection_criterion, allow_rotations,
                                               preselect, include_independence, pair_kwargs, strict, trace, threaded,).copula
end
