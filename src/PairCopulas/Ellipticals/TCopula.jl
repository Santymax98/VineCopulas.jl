struct _StudentPairKernel{R<:Real,D} <: Copulas.Copula{2}
    rho::R
    nu::D
end
...
@inline _t_quantile(::Val{ν}, p::Union{Float32,Float64}) where {ν} = Rmath.qt(Float64(_clp(p)), Float64(ν))
@inline _t_quantile(::Val{ν}, p::Real) where {ν} = StatsFuns.tdistinvcdf(Float64(ν), _clp(p))
@inline _t_cdf(::Val{ν}, x::Union{Float32,Float64}) where {ν} = Rmath.pt(Float64(x), Float64(ν))
@inline _t_cdf(::Val{ν}, x::Real) where {ν} = StatsFuns.tdistcdf(Float64(ν), x)
@inline _t_quantile(ν::Real, p::Union{Float32,Float64}) = Rmath.qt(Float64(_clp(p)), Float64(ν))
@inline _t_quantile(ν::Real, p::Real) = StatsFuns.tdistinvcdf(Float64(ν), _clp(p))
@inline _t_cdf(ν::Real, x::Union{Float32,Float64}) = Rmath.pt(Float64(x), Float64(ν))
@inline _t_cdf(ν::Real, x::Real) = StatsFuns.tdistcdf(Float64(ν), x)

@generated function _t_pair_K(::Val{ν}) where {ν}
    νf = Float64(ν)
    univ_const = SpecialFunctions.loggamma((νf + 1) / 2) - SpecialFunctions.loggamma(νf / 2) - 0.5 * log(νf * π)
...
@inline function _t_logpdf_from_quantiles(df, ρ::Real, den::Real, t1::Real, t2::Real,)
    νf = _t_float(df)
    Q = t1 * t1 - 2 * ρ * t1 * t2 + t2 * t2
    return _t_pair_K(df) - 0.5 * log(den) - ((νf + 2) / 2) * log1p(Q / (νf * den)) + ((νf + 1) / 2) * (log1p((t1 * t1) / νf) + log1p((t2 * t2) / νf))
end

@inline function _t_h_from_quantiles(df, ρ::Real, den::Real, target_t::Real, base_t::Real,)
    νf = _t_float(df)
    scale = sqrt((νf + base_t * base_t) * den / (νf + 1))
    return _t_cdf(_t_plus_one(df), (target_t - ρ * base_t) / scale)
end

@inline function _pair_logpdf(C::_StudentPair, u::Real, v::Real, buf::Vector{Float64},)
    df, ρ, den, t1, t2 = _t_pair_inputs(C, u, v)
    return _t_logpdf_from_quantiles(df, ρ, den, t1, t2)
end

@inline function _pair_hfuncs(C::_StudentPair, u::Real, v::Real,)
    df, ρ, den, t1, t2 = _t_pair_inputs(C, u, v)
...
@inline function _pair_step(C::_StudentPair, u::Real, v::Real, buf::Vector{Float64},)
    df, ρ, den, t1, t2 = _t_pair_inputs(C, u, v)
...
@inline function _pair_logpdf_h1(C::_StudentPair, u::Real, v::Real, buf::Vector{Float64},)
    df, ρ, den, t1, t2 = _t_pair_inputs(C, u, v)
    logc = _t_logpdf_from_quantiles(df, ρ, den, t1, t2)
    h1 = _clp(_t_h_from_quantiles(df, ρ, den, t1, t2))
    return logc, h1
...
@inline function _pair_logpdf_h2(C::_StudentPair, u::Real, v::Real, buf::Vector{Float64},)
    df, ρ, den, t1, t2 = _t_pair_inputs(C, u, v)
    logc = _t_logpdf_from_quantiles(df, ρ, den, t1, t2)
    h2 = _clp(_t_h_from_quantiles(df, ρ, den, t2, t1))
    return logc, h2
...
@inline function _t_hfunc(C::_StudentPair, target::Real, base::Real,)
    ρ = _student_rho(C)
    ρ2 = ρ * ρ
    den = one(ρ2) - ρ2
    df = _tcopula_df(C)
...
@inline function _t_hinv(C::_StudentPair, q::Real, base::Real,)
    df = _tcopula_df(C)
    νf = _t_float(df)
    ρ = _student_rho(C)
    ρ2 = ρ * ρ
...
@inline hfunc1(C::_StudentPair, u::Real, v::Real) = _clp(_t_hfunc(C, u, v))
# hfunc2(C,u,v) = C_{2|1}(v | u)
@inline hfunc2(C::_StudentPair, u::Real, v::Real) = _clp(_t_hfunc(C, v, u))
@inline hfunc1(C::_StudentPair, uv::Tuple{<:Real,<:Real}) = hfunc1(C, uv[1], uv[2])
@inline hfunc2(C::_StudentPair, uv::Tuple{<:Real,<:Real}) = hfunc2(C, uv[1], uv[2])
# Inverse of hfunc1: given q = C_{1|2}(u | v), recover u.
@inline hinv1(C::_StudentPair, q::Real, v::Real) = _clp(_t_hinv(C, q, v))
# Inverse of hfunc2: given q = C_{2|1}(v | u), recover v.
@inline hinv2(C::_StudentPair, q::Real, u::Real) = _clp(_t_hinv(C, q, u))
