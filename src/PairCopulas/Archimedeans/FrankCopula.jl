...
@inline function _pair_logpdf(C::Copulas.FrankCopula{2}, u::Real, v::Real, ::Vector{Float64},)
    return _frank_pair_logpdf(C, _clp(u), _clp(v),)
end
...
@inline function hfunc1(C::Copulas.FrankCopula{2}, u::Real, v::Real,)
    return _clp(_frank_hfunc(C, _clp(u), _clp(v),),)
end

@inline function hfunc2(C::Copulas.FrankCopula{2}, u::Real, v::Real,)
    return _clp(_frank_hfunc(C, _clp(v), _clp(u),),)
end
...
@inline function hinv1(C::Copulas.FrankCopula{2}, q::Real, v::Real,)
    return _clp(_frank_hinv(C, _clp(q), _clp(v),),)
end

@inline function hinv2(C::Copulas.FrankCopula{2}, q::Real, u::Real,)
    return _clp(_frank_hinv(C, _clp(q), _clp(u),),)
end
