...
#     exp(x_target) = exp(x_total) - exp(x_base) + c.
...
@inline function _arch_pair_logpdf(C::Copulas.BB9Copula{2}, u::Real, v::Real)
    ...
    return _arch_logderivative(C, x) + logslope - x - _arch_logderivative(C, xu) - _arch_logderivative(C, xv)
end

@inline _pair_logpdf(C::Copulas.BB9Copula{2}, u::Real, v::Real, ::Vector{Float64}) = _arch_pair_logpdf(C, _clp(u), _clp(v))
@inline hfunc1(C::Copulas.BB9Copula{2}, u::Real, v::Real) = _clp(_arch_hfunc(C, _clp(u), _clp(v)))
@inline hfunc2(C::Copulas.BB9Copula{2}, u::Real, v::Real) = _clp(_arch_hfunc(C, _clp(v), _clp(u)))
@inline hinv1(C::Copulas.BB9Copula{2}, q::Real, v::Real) = _clp(_arch_hinv(C, _clp(q), _clp(v)))
@inline hinv2(C::Copulas.BB9Copula{2}, q::Real, u::Real) = _clp(_arch_hinv(C, _clp(q), _clp(u)))
