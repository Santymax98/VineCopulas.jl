...
@inline function _joe_logpdf_from_terms(θ::Real, x::Real, y::Real, log1mA::Real, log1mB::Real, logS::Real,)
    ...
    α = inv(θ)
    # c(u,v) =  θ (1-u)^(θ-1) (1-v)^(θ-1) S^(1/θ-2) × [S + (1-1/θ)(1-A)(1-B)].
    logR = LogExpFunctions.logaddexp(logS, log1p(-α) + log1mA + log1mB,)
    return log(θ) - (θ - one(θ)) * (x + y) + (α - 2) * logS + logR
end
...
@inline function _joe_logh2_from_terms(θ::Real, x::Real, log1mB::Real, logS::Real,)
    ...
    α = inv(θ)
    return log1mB - (θ - one(θ)) * x + (α - one(θ)) * logS
end
...
@inline _pair_logpdf(C::Copulas.JoeCopula{2}, u::Real, v::Real, ::Vector{Float64},) = _joe_pair_logpdf(C, _clp(u), _clp(v),)
...
@inline hfunc1(C::Copulas.JoeCopula{2}, u::Real, v::Real,) = _clp(_joe_hfunc(C, _clp(u), _clp(v),),)
@inline hfunc2(C::Copulas.JoeCopula{2}, u::Real, v::Real,) = _clp(_joe_hfunc(C, _clp(v), _clp(u),),)
...
@inline hinv1(C::Copulas.JoeCopula{2}, q::Real, v::Real,) = _clp(_joe_hinv(C, _clp(q), _clp(v),),)
@inline hinv2(C::Copulas.JoeCopula{2}, q::Real, u::Real,) = _clp(_joe_hinv(C, _clp(q), _clp(u),),)
