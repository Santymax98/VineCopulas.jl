...
@testsnippet ThreadedFit begin
    ...
    using StatsBase: coef, dof, vcov, stderror
    using Copulas
    using Copulas: infer
    using VineCopulas
    using StableRNGs
    ...
    # One entry point for the four fitters under test: the pair selector is
    # `select_paircopula`, the vines go through `fit`.
    vfit(::Type{PairCopula}, X; kw...) = select_paircopula(X; kw...)
    vfit(::Type{T}, X; kw...) where {T<:AbstractVineCopula} = fit(T, X; kw...)
end
...
@testitem "Threaded fit – identical to the sequential fit" tags=[:Fit, :Vine, :Threads] setup=[ThreadedFit] begin
    ...
    # The statistical model: the same vine, log-likelihood and coefficients,
    # and the same recipe, since `threaded` says how the fit ran and not what
    # it estimated. The bootstrap refit walks the fitted vine, so `infer`
    # runs on the threaded model as on the sequential one.
    for T in (RVineCopula, CVineCopula, DVineCopula)
        local Ms = fit(VineModel, T, U; family_set=fams, trunc=2)
        local Mt = fit(VineModel, T, U; family_set=fams, trunc=2, threaded=true)
        @test deq(Copulas.fitted_distribution(Ms), Copulas.fitted_distribution(Mt))
        @test loglikelihood(Ms) == loglikelihood(Mt)
        @test coef(Ms) == coef(Mt)
        @test Mt.recipe == Ms.recipe
        @test !haskey(Mt.recipe.kwargs, :threaded)
        local I = infer(Mt; nresamples=3, rng=StableRNG(45))
        @test size(vcov(I)) == (dof(Mt), dof(Mt))
        @test all(isfinite, stderror(I))
    end

    # The pair selector alone: the candidate level with no edge level.
    U2 = U[1:2, :]
    @test deq(select_paircopula(U2), select_paircopula(U2; threaded=true))
    @test deq(
        select_paircopula(U2; family_set=:all, selection_criterion=:loglik),
        select_paircopula(U2; family_set=:all, selection_criterion=:loglik, threaded=true),
    )
    ...
    s = select_paircopula(U2; family_set=(ClaytonCopula, ClaytonCopula, FrankCopula), include_independence=false)
    t = select_paircopula(U2; family_set=(ClaytonCopula, ClaytonCopula, FrankCopula), include_independence=false, threaded=true)
    @test deq(s, t)
...
@testitem "Threaded fit – errors are the sequential errors" tags=[:Fit, :Vine, :Threads, :Validation] setup=[ThreadedFit] begin
    ...
    for T in (PairCopula, RVineCopula, CVineCopula, DVineCopula)
        ...
        local es = thrown(() -> vfit(T, X; strict=true, bad...))
        local et = thrown(() -> vfit(T, X; strict=true, threaded=true, bad...))
    ...
    es = thrown(() -> select_paircopula(U2; bad...))
    et = thrown(() -> select_paircopula(U2; threaded=true, bad...))
...
@testitem "Threaded fit – trace lines match in edge order" tags=[:Fit, :Vine, :Threads] setup=[ThreadedFit] begin
    ...
    for (T, kw) in ((RVineCopula, (; family_set=fams)),
        ...
        seq = captured(() -> vfit(T, X; trace=true, kw...))
        thr = captured(() -> vfit(T, X; trace=true, threaded=true, kw...))
        @test !isempty(seq)
    ...
    seq = captured(() -> thrown(() -> select_paircopula(U2; bad...)))
    thr = captured(() -> thrown(() -> select_paircopula(U2; threaded=true, bad...)))
